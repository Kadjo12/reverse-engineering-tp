...
typedef struct {
    char username[16];
    unsigned char password_hash[20]; // SHA-1 hash
    int admin_flag;
    int balance;
    char *token;
} User;
...
void register_user() {
    ...
    SHA1((unsigned char *)password, strlen(password), users[user_count].password_hash);
...
int login_user(char *username, char *password) {
    for(int i = 0; i < user_count; i++) {
        if(strcmp(users[i].username, username) == 0 && 
           verify_password(password, users[i].password_hash)) {
            return i;
        }
    }
    return -1;
...
void system_command() {
    ...
    void system_command(void) { 
        printf("Cette fonctionnalité est désactivée pour des raisons de sécurité.\n"); }
}
...
void clone_user(int source_id) {
    ...
    memcpy(users[user_count].password_hash, users[source_id].password_hash, 20);
...
void change_password(int user_id) {
    ...
    if (verify_password(old_pass, users[user_id].password_hash)) { 
        printf("Nouveau mot de passe: ");
        gets(new_pass);
        SHA1((unsigned char *)new_pass, strlen(new_pass), users[user_id].password_hash);
